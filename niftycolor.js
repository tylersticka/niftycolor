var niftyColor = {
  ...
  patterns: {
    ...
    rgba: /rgba\(\s*(\d+\%?),\s*(\d+\%?),\s*(\d+\%?),\s*(\d*\.?\d*)\s*\)/,
    hsl: /hsl\(\s*(\d+),\s*(\d+)\%,\s*(\d+)\%\s*\)/,
    hsla: /hsla\(\s*(\d+),\s*(\d+)\%,\s*(\d+)\%,\s*(\d*\.?\d*)\s*\)/,
    parenthetical: /\(.*\)/
  }
};
...
niftyColor.hexToArray = function (str) {
  ...
  str = str.replace('#', '', str);
  if (str.length === 3) {
    for (shorthand=str, str=''; i < 3; i++) {
      str += shorthand[i] + shorthand[i];
    };
  }
  hex = parseInt(str, 16);
  return [
...
niftyColor.rgbToArray = function (str, pattern) {
  var arr = str.match(pattern || this.patterns.rgb);
  if (arr) {
    arr = this._numericArray(arr.slice(1));
...
niftyColor.rgbaToArray = function (str) {
  return this.rgbToArray(str, this.patterns.rgba);
};

niftyColor.hslToArray = function (str, pattern) {
  var matches = str.match(pattern || this.patterns.hsl)
    , h, s, l
    , r, g, b
    , temp1, temp2;
  if (matches) {
    matches = this._numericArray(matches.slice(1));
    h = matches[0] / 360;
    s = matches[1] / 100;
    l = matches[2] / 100;
    if (s > 0) {
...
niftyColor.parse = function (str) {
  ...
  if (str.match(this.patterns.hex)) {
    notation = 'hex';
    arr = niftyColor.hexToArray(str);
  } else if (str.match(this.patterns.parenthetical)) {
    notation = str.substr(0, str.indexOf('('));
    method = notation + 'ToArray';
    if (niftyColor[method]) {
      arr = niftyColor[method](str);
    }
  } else if (this.keywords[str]) {
    notation = 'keyword';
